...
if [[ "$sispopnet_time" != "" && "$storage_time" != "" ]]; then
  if [[ "$sispopnet_time" -ge 305 ]]; then
    ...
    echo -e "Performing your action because Sispopnet last ping time is greater than 305 seconds." >> /proc/1/fd/1
    ...
    echo -e "Sispopnet last ping time is within acceptable range: $sispopnet_time seconds." >> /proc/1/fd/1
  fi

  if [[ "$storage_time" -ge 305 ]]; then
    echo "Performing your action because Storage last ping time is greater than 305 seconds."
    echo "Performing your action because Storage last ping time is greater than 305 seconds." >> /proc/1/fd/1
    supervisorctl restart storage > /dev/null 2>&1
  else
    echo -e "Storage last ping time is within acceptable range: $storage_time seconds."
    echo -e "Storage last ping time is within acceptable range: $storage_time seconds."  >> /proc/1/fd/1
  fi
fi
